...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return  accounts.stream().filter(account -> birthdayMonth.equals(account.getBirthday().getMonth())).toList();
    }
    ...
    public BigDecimal getBalanceByEmailDomain(String email) {
        return accounts.stream().filter(account -> account.getEmail().split("@")[1].equals(email)).map(Account::getBalance).reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream().collect(Collectors.groupingBy(Account::getLastName, Collectors.mapping(Account::getFirstName, Collectors.toSet())));
    }
